...
export default function Tracker() {
  ...
  const handleJSONUpload = async () => {
    ...
    if (
      !window.confirm(
        "⚠️ WARNING: This will completely WIPE the current database and replace it with your JSON file. Any live 'entered' tracking will be reset! Proceed?",
      )
    )
      return;
  ...
  const filteredGuests = guests.filter((guest) => {
    // 1. Hide the ADMIN BOSS from the tracker list completely
    if (guest.usn === "ADMIN_BOSS_USN" || guest.name === "ADMIN BOSS") {
      return false;
    }

    // 2. Keep standard search behavior for everyone else
    return (
    ...
    <div className="min-h-screen pb-24 pt-8 px-4 flex flex-col items-center">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 mb-6 border border-white/10 text-center shadow-lg">
          <h1 className="text-2xl font-extrabold text-transparent bg-clip-text bg-linear-to-r from-purple-400 via-pink-500 to-red-500 mb-2">
            Live Entry Tracker
          </h1>
          <div className="flex justify-center space-x-8 text-white mt-4">
            <div>
              <p className="text-3xl font-bold text-cyan-400">{totalGuests-1}</p>
              <p className="text-xs text-purple-300 uppercase tracking-wide font-bold">
                Total
              </p>
            </div>
            <div>
            ...
            filteredGuests.map((guest) => (
              ...
              >
                <div className="flex items-center">
                  ...
                  />
                  <div>
                    <p className="font-bold text-cyan-400">{guest.name}</p>
                ...
                {currentUser?.isAdmin ? (
                  ...
                  <div
                    className={`${guest.entered ? "text-green-400 drop-shadow-[0_0_8px_rgba(34,197,94,0.5)]" : "text-gray-500"}`}
                  >
                    {guest.entered ? (
                      <CheckCircle2 size={28} />
                    ) : (
                      <span className="text-[10px] font-bold px-2 py-1 bg-black/50 border border-white/10 rounded uppercase tracking-wider text-gray-400">
                        Waiting
                      </span>
                    )}
                  </div>
